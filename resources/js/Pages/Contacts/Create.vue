      ...
      <form @submit.prevent="store">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <select-input v-model="form.country" :error="form.errors.country" class="pr-6 pb-8 w-full lg:w-1/2" label="Šalis">
            <option :value="null" />
            <option value="CA">Latvija</option>
            <option value="US">Lietuva</option>
          </select-input>
          <text-input v-model="form.plan" :error="form.errors.plan" class="pr-6 pb-8 w-full lg:w-1/2" label="Planas" />
        ...
        <div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex justify-end items-center">
          <loading-button :loading="form.processing" class="btn-indigo" type="submit">Sukurti kontaktą</loading-button>
        </div>
      </form>
    </div>
  </div>
